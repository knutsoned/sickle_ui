...
impl UiUtils {
    ...
    pub fn simplify_type_name(name: &str) -> String {
        ...
        if name.split("<").count() > 1 {
            ...
            let generic = name.split("<").skip(1).next().unwrap().split("::").last().unwrap();
    ...
    pub fn container_size_and_offset(entity: Entity, world: &World) -> (Vec2, Vec2) {
        ...
        let mut offset = world.get::<GlobalTransform>(entity).unwrap().translation().truncate();
        ...
        while let Some(parent) = world.get::<Parent>(current_ancestor) {
            ...
            if container_size.x == 0.0 && style.overflow.x == OverflowAxis::Clip {
                container_size.x = node_size.x;
                offset.x -= current_pos.x - node_size.x / 2.0;
            }

            if container_size.y == 0.0 && style.overflow.y == OverflowAxis::Clip {
                container_size.y = node_size.y;
                offset.y -= current_pos.y - node_size.y / 2.0;
            }

            if container_size.x > 0.0 && container_size.y > 0.0 {
        ...
        if let Some(render_target) = UiUtils::find_render_target(entity, world) {
            ...
            container_size = UiUtils::resolution_to_vec2(
    ...
    pub fn val_to_px(value: Val, parent: f32, viewport_size: Vec2) -> f32 {
        match value {
            ...
            Val::Percent(percent) => ((parent * percent) / 100.0).max(0.0),
            Val::Vw(percent) => ((viewport_size.x * percent) / 100.0).max(0.0),
            Val::Vh(percent) => ((viewport_size.y * percent) / 100.0).max(0.0),
            Val::VMin(percent) => ((viewport_size.min_element() * percent) / 100.0).max(0.0),
            Val::VMax(percent) => ((viewport_size.max_element() * percent) / 100.0).max(0.0),
        }
    }
